...
$.fn.fadeJQ = function(startcolor,endcolor) {
	
	
	//////////////////////////////////// VARS ////////////////////////////////////////////////////////
	var doFade_fadeInterval=400;// The time in miliseconds between color steps
	var doFade_fadeSteps=7;// The number of different shades during fade
	var doFade_useExistingAsEnd=true; // Use the existing background color as the end color if a background color is not provided
	var doFade_defaultStartColor="ffff99"; //default starting color if one is not provided
	var doFade_defaultEndColor="ffffff"; //default ending color if one is not provided and do_FadeuseExistingAsEnd is false;
	...
	if(typeof startcolor=="object"){
		...
		if(paramObj.steps!=undefined)doFade_fadeSteps=paramObj.steps;
		if(paramObj.interval!=undefined)doFade_fadeInterval=paramObj.interval;
	...
	getBackground=function(){
		...
		if(bg=="") return doFade_defaultEndColor; // no background color set - use default 
	...
	//
	//Called doFade_fadeSteps times through recursion
	/////////////////////////////////////
	doFadeMain=function(element,startcolor,endcolor,step,startr,startg,startb,endr,endg,endb){
		
		//No start color provided - use default
		//Note: the start color is set on the first pass of this method.
		if(startcolor==undefined)startcolor=doFade_defaultStartColor;
		...
		if(endcolor==undefined){
			if(doFade_useExistingAsEnd)endcolor=getBackground();
			else endcolor=doFade_defaultEndColor;
		}
		...
		if(step<doFade_fadeSteps){
			var r=Math.round(startr+(endr-startr)/doFade_fadeSteps*step);
			var g=Math.round(startg+(endg-startg)/doFade_fadeSteps*step);
			var b=Math.round(startb+(endb-startb)/doFade_fadeSteps*step);
			...
			setTimeout(function(){
			 doFadeMain(element,startcolor,endcolor,step,startr,startg,startb,endr,endg,endb);
	...
	doFadeMain(element,startcolor,endcolor); // Call the main method 
